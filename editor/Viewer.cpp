...
{
	// Use our own OnFileOpen method so we can
	// implement our own file filters.

	static char szFilter[] = "Quake III Model Files (*.md3)|*.md3||";
	CFileDialog fileName( TRUE, "*.md3", NULL,
		OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, szFilter );

	// Stop the load process if the user clicked
	// cancel.
	if ( fileName.DoModal() != IDOK )
		return;

	// Continue using the framework to load the
	// document.
